...
public WorkTracker(CuratorFramework curatorClient,
    ...
    public void forceAssignment(Subscription subscription) {
        askCuratorPolitely(() -> curatorClient.create().creatingParentsIfNeeded().withMode(PERSISTENT).forPath(pathSerializer.serialize(subscription.toSubscriptionName(), consumerNodeId)));
    }
